...
#    compare the impact of the initial population
using DataFrames
using CSV
using Printf 
...
for (j,measure) in enumerate(["Generation", "Nodes"])
    ...
    for g_0 in Gs
        ...
        Plots.plot!( p2[j], x, DF[:, y_symbol_1] ; label=L"g_0=%$g_0")
    ...
    global plot_file = joinpath( plot_dir, @sprintf("random_surreal_sim_%s_%0.2f_%0.2f_%02d_proc_3b_%s.pdf", 

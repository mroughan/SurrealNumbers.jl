...
######## FIRST PICTURE
file = joinpath(@__DIR__, out_dir, "dyadic_tree_$n.dot")
io = open(file, "w")
...
for k=1:n
    ...
    println(" k = $k, x = $x")
    tmp = name_map[x.R[1]]
    println(io, "   node_$m:R -> node_$tmp [color=\"blue3\", dir=$direction];")
    ...
    println(" k = $k, x = $x")
    tmp = name_map[x.L[1]] 
    println(io, "   node_$m:L -> node_$tmp [color=\"red3\", dir=$direction];")
